...
import { DeliveryAddressApi } from '../api/endpoints/delivery-address-api';
...
export const useLocation = () => {
  ...
  const { state } = useAuth();
  const deliveryAddressApi = useMemo(() => new DeliveryAddressApi(apiConfig), []);
  ...
  /**
   * Get default delivery address from API
   */
  const getDefaultAddress = useCallback(async (): Promise<LocationCoords | null> => {
    ...
    try {
      const defaultResp = await deliveryAddressApi.deliveryAddressMeDefaultGet();
      const def = defaultResp?.data;

      if (def?.latitude != null && def?.longitude != null) {
        const lat = def.latitude as number;
        const lon = def.longitude as number;

        // Validate coordinates
        if (lat >= -90 && lat <= 90 && lon >= -180 && lon <= 180 && !(lat === 0 && lon === 0)) {
          setLocation({ latitude: lat, longitude: lon });

          const label = formatAddress(def);
          setLocationAddress(label || `${lat.toFixed(4)}, ${lon.toFixed(4)}`);

          return { latitude: lat, longitude: lon };
        } else {
          console.warn('Invalid coordinates from default address:', lat, lon);
        }
      }
      return null;
    } catch (error) {
  ...
  }, [state.isAuthenticated, deliveryAddressApi, formatAddress]);
  ...
  const initializeLocation = useCallback(async (showGoogleMapsInfo: boolean = false) => {
    ...
    try {
      ...
      // Strategy 1: Try default delivery address first
      const defaultLocation = await getDefaultAddress();
      if (defaultLocation) {
        setIsLoadingLocation(false);
        return defaultLocation;
      }

      // Strategy 2: Fallback to device location with retry
  ...
  }, [getDefaultAddress, getCurrentLocationWithRetry, updateLocationFromCoords, showError, showInfo]);
