...
const AddressAutocompleteEnhanced: React.FC<AddressAutocompleteEnhancedProps> = ({
  ...
  const performSearch = useCallback(async (searchQuery: string) => {
    ...
    try {
      const results = await searchPlacesGoogle(
        searchQuery,
  ...
  setSelectedIndex(index);
    ...
    if (requireConfirmation) {
      try {
        ...
        // Notify parent of the selected address. Do NOT clear the parent's
  ...
  onAddressSelect(selected);
    ...
    try {
      ...
      if (!onCreateAddress) {
        console.warn('onCreateAddress is not provided');
        return;
      }
  ...
  const handleInputBlur = useCallback(() => {
    setIsFocused(false);
    // Delay hiding suggestions to allow taps on them to register.
    // Without this, the blur event can hide the list before the press is handled.
    // Increase the delay slightly to improve reliability on Android devices
    // where taps may take longer to register.
    if (blurTimeoutRef.current) {
      clearTimeout(blurTimeoutRef.current);
    }
    ...
    <View style={containerStyles}>
      ...
      >
        ...
        <TextInput
          ...
          placeholderTextColor={searchError ? "#FF9999" : "#999"}
          value={inputValue}
      ...
      {showSuggestions && suggestions.length > 0 && (
        <View style={styles.suggestionsContainer}>
          <ScrollView
            style={{ maxHeight: 280 }}
            // Ensure taps on suggestion items are always delivered even when
            // the TextInput blurs on Android. 'always' improves reliability
            // for touch devices.
...
const styles = StyleSheet.create({
  ...
  inputContainerFocused: {
    ...
    ...(Platform.select({
      ...
      web: {
        boxShadow: '0px 0px 4px rgba(244, 128, 34, 0.1)'
      }
    }) as any),
  },
  inputContainerError: {
