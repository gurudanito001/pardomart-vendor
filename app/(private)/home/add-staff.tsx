import { Role, StaffApi, type StaffPostRequest, Vendor } from '@/api';
...
export default function AddStaffScreen() {
  ...
  const [selectedStoreId, setSelectedStoreId] = useState<string | undefined>(storeIdFromParam);
  const [selectedRole, setSelectedRole] = useState<Role | undefined>();

  const [isModalVisible, setIsModalVisible] = useState(false);
  const [modalConfig, setModalConfig] = useState<{ title: string; options: { label: string; value: any }[]; onSelect: (value: any) => void; selectedValue: any; } | null>(null);
  ...
  const vendorOptions = useMemo(() => {
    ...
    return vendors
      .filter((v: Vendor) => typeof v.id === 'string')
      .map((v: Vendor) => ({ label: v.name ?? 'Unnamed Store', value: v.id! }));
  }, [vendorsData]);

  const selectedStoreName = useMemo(() => {
    return vendorOptions.find(v => v.value === selectedStoreId)?.label ?? 'Select a store';
  }, [vendorOptions, selectedStoreId]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ['staff', selectedStoreId] });
      router.replace({ pathname: '/(private)/home/my-staff', params: { storeId: selectedStoreId } } as any);
    },
    onError: (e: any) => {
  ...
  const canSubmit = name.trim().length > 0 && email.trim().length > 0 && mobileNumber.trim().length > 0 && !!selectedStoreId && !!selectedRole;
  ...
  const openRoleSelector = () => {
    setModalConfig({ title: 'Select a Role', options: roleOptions, onSelect: (value) => setSelectedRole(value), selectedValue: selectedRole });
    setIsModalVisible(true);
  };
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.form}>
            <View style={styles.fieldGroup}>
              ...
              <TouchableOpacity style={styles.selectField} onPress={openStoreSelector}>
                <Text style={styles.selectFieldText}>
                  {isLoadingStores ? 'Loading stores...' : selectedStoreName}
                </Text>
            ...
            <View style={styles.fieldGroup}>
              ...
              <TouchableOpacity style={styles.selectField} onPress={openRoleSelector}>
                <Text style={styles.selectFieldText}>
                  {selectedRoleName}
                </Text>
                <Svg width="12" height="8" viewBox="0 0 12 8" fill="none"><Path d="M1 1.5L6 6.5L11 1.5" stroke="#000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></Svg>
            ...
            <View style={styles.modalContent} onStartShouldSetResponder={() => true}>
              ...
              <ScrollView>
                {modalConfig.options.map((opt) => (
                  <TouchableOpacity
                    key={opt.value}
                    style={[styles.optionItem, modalConfig.selectedValue === opt.value && styles.selectedOption]}
                    onPress={() => {
                      modalConfig.onSelect(opt.value);
                      setIsModalVisible(false);
                    }}
                  >
                    <Text style={[styles.optionText, modalConfig.selectedValue === opt.value && styles.selectedOptionText]}>
                      {opt.label}
                    </Text>
                  </TouchableOpacity>
                ))}
