...
export default function TabLayout() {
  ...
  // Determine if the user is a store admin or shopper
  const isStoreUser = userRole === Role.StoreAdmin || userRole === Role.StoreShopper;
  // Check if the current route is a store-specific page (e.g., /(private)/store/...)
  const isInStorePages = segments.includes('store');

  const activeColor = "#06888C";
  const inactiveColor = "#484C52";
    ...
    <Tabs
      ...
      <Tabs.Screen
        ...
        options={{
          title: "Home",
          tabBarIcon: ({ focused }) => {
            const iconColor = (focused || (isStoreUser && isInStorePages)) ? activeColor : inactiveColor;
            return (
          ...
          tabBarLabel: ({ focused }) => {
            const labelColor = (focused || (isStoreUser && isInStorePages)) ? activeColor : inactiveColor;
            return (
              <Text
                style={{
                  fontSize: 10,
        ...
        listeners={{
          tabPress: (e) => {
            e.preventDefault();
            if (vendorId) {
              router.replace(`/(private)/home?storeId=${vendorId}`);
            } else {
      ...
      />
        ...
        name="store"
        options={{
          ...(state.user?.role !== Role.Vendor && {href: null}),
          title: "Stores",
          tabBarIcon: ({ color, size }) => (
            <View style={{ width: 24, height: 24 }}>
