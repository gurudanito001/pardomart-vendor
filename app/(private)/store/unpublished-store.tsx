...
import type { VendorProduct, Vendor } from '../../../api/models';



export default function UnpublishedStoreScreen() {
  const { storeId } = useLocalSearchParams<{ storeId?: string }>();
  const vendorId = String(storeId || '');

  const [selectedTab, setSelectedTab] = useState<'shop' | 'pickup' | 'delivery'>('pickup');
  ...
  useEffect(() => {
    if (gridWidth > 0) {
      const possible = Math.max(2, Math.floor((gridWidth + GRID_GAP) / (MIN_CARD_WIDTH + GRID_GAP)));
      setColumns(possible);
    }
  }, [gridWidth]);
  ...
  const availableTypes = useMemo(() => {
    const meta = (vendor?.meta || {}) as any;
    const types: string[] = Array.isArray(meta?.deliveryTypes) ? meta.deliveryTypes : ['shop', 'pickup', 'delivery'];
    return {
      shop: types.includes('shop') || types.includes('shop_and_deliver') || types.includes('shopAndDeliver'),
      pickup: types.includes('pickup'),
      delivery: types.includes('delivery') || types.includes('delivery_person') || types.includes('deliveryPerson'),
    };
  }, [vendor]);

  // Fetch vendor details for header and delivery types
  useEffect(() => {
    let isMounted = true;
    const loadVendor = async () => {
      ...
      try {
        ...
        const api = vendorApi();
        const res = await api.vendorsIdGet(vendorId);
        if (isMounted) setVendor(res?.data ?? null);
      } catch {
  ...
  useEffect(() => {
    // Pick first available type as default
    if (availableTypes.shop) setSelectedTab('shop');
    else if (availableTypes.pickup) setSelectedTab('pickup');
    else if (availableTypes.delivery) setSelectedTab('delivery');
  }, [availableTypes.shop, availableTypes.pickup, availableTypes.delivery]);
  ...
  const handleAddProduct = () => {
    if (!vendorId) return;
    router.push(`/(private)/store/select-category?storeId=${encodeURIComponent(vendorId)}` as any);
  };
  ...
  const renderProduct = (vp: VendorProduct, cardWidthOverride?: number) => {
    const img = vp.images && vp.images.length > 0 ? vp.images[0] : undefined;
    const hasDiscount = vp.discountedPrice !== null && vp.discountedPrice !== undefined && vp.discountedPrice < (vp.price ?? 0);
    const sizeText = vp.weight && vp.weightUnit ? `${vp.weight} ${vp.weightUnit}` : '';
    return (
      <View key={vp.id} style={[styles.productCard, cardWidthOverride ? { width: cardWidthOverride } : null]}>
        <View style={styles.productImageContainer}>
          {img ? (
            <Image source={{ uri: img }} style={styles.productImage} resizeMode="contain" />
          ) : (
            <Svg width="40" height="40" viewBox="0 0 24 24" fill="none">
              <Path d="M3 7h18l-1.5 9.5A3 3 0 0 1 16.53 19H7.47A3 3 0 0 1 4.5 16.5L3 7Z" fill="#D9D9D9" />
            </Svg>
          )}
        </View>
          ...
          <View style={styles.priceContainer}>
            {hasDiscount && (
              <View style={styles.discountBadge}>
                <Text style={styles.discountText}>${(vp.discountedPrice as number).toFixed(2)}</Text>
              </View>
            )}
            <Text style={styles.productPrice}>${(vp.price ?? 0).toFixed(2)}</Text>
          </View>
          <Text style={styles.productName} numberOfLines={2}>
            {vp.name}
          </Text>
          {!!sizeText && <Text style={styles.productSize}>{sizeText}</Text>}
        </View>
      </View>
    );
  };
  ...
  // Products fetching with pagination and search
  const fetchProducts = useCallback(async (page: number, limit: number) => {
    ...
    const res = await api.productVendorGet(debouncedQuery || undefined, vendorId, undefined, undefined, undefined, page, limit);
    const payload = res?.data ?? ({} as any);
    const currentPage = payload.page != null ? (Number(payload.page) + 1) : (page ?? 1);
    ...
    const items: VendorProduct[] = Array.isArray(payload.data) ? payload.data : [];
    return {
      data: {
        items,
        pagination: {
  ...
  }, [vendorId, debouncedQuery]);

  const { items: products, pagination, loading, refreshing, error, loadMore, refresh } = usePaginatedApi<VendorProduct>(fetchProducts, 9, true);

  // Refetch when search or vendor changes
  useEffect(() => {
    if (vendorId) {
      refresh();
    }
  }, [vendorId, debouncedQuery, refresh]);
    ...
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.headerContent}>
          ...
          <View style={styles.storeInfoSection}>
            <View style={styles.storeLogoContainer}>
              {vendor?.image ? (
                <Image source={{ uri: vendor.image }} style={styles.headerStoreLogo} resizeMode="contain" />
              ) : (
                <Svg width="40" height="40" viewBox="0 0 24 24" fill="none">
                  <Path d="M3 7h18l-1.5 9.5A3 3 0 0 1 16.53 19H7.47A3 3 0 0 1 4.5 16.5L3 7Z" fill="#FFF" />
                </Svg>
              )}
            </View>
            <Text style={styles.storeLocation} numberOfLines={2}>{vendor?.address || ' '}</Text>
          </View>
      ...
      <View style={styles.overlayContainer}>
        ...
        <View style={styles.tabSection}>
          <View style={styles.tabContainer}>
            <TouchableOpacity
              style={[styles.tab, selectedTab === 'shop' && styles.activeTab, !availableTypes.shop && { opacity: 0.4 }]}
              onPress={() => availableTypes.shop && setSelectedTab('shop')}
              disabled={!availableTypes.shop}
            >
            ...
            <TouchableOpacity
              style={[styles.tab, selectedTab === 'pickup' && styles.activeTab, !availableTypes.pickup && { opacity: 0.4 }]}
              onPress={() => availableTypes.pickup && setSelectedTab('pickup')}
              disabled={!availableTypes.pickup}
            >
              <Svg width="14" height="14" viewBox="0 0 14 15" fill="none">
            ...
            <TouchableOpacity
              style={[styles.tab, selectedTab === 'delivery' && styles.activeTab, !availableTypes.delivery && { opacity: 0.4 }]}
              onPress={() => availableTypes.delivery && setSelectedTab('delivery')}
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.content}>
          {/* Products Header */}
          <Text style={styles.productsHeader}>Products ({pagination?.total ?? products.length})</Text>

          {/* Empty state */}
          {!loading && products.length === 0 ? (
            <View style={styles.emptyState}>
              <Text style={styles.emptyTitle}>No products found</Text>
              <Text style={styles.emptySubtitle}>Try a different search or add a new product.</Text>
            </View>
          ) : (
            <>
              {/* Products Grid */}
              <View style={styles.productsGrid} onLayout={({ nativeEvent }) => setGridWidth(nativeEvent.layout.width)}>
                <View style={styles.productsWrap}>
                  {products.map((p) => renderProduct(p, cardWidth))}
                </View>
              </View>
              ...
              {products.length > 0 && (
                <View style={styles.pagination}>
                  <Text style={styles.paginationText}>
                    {`Showing ${(pagination.page - 1) * pagination.limit + Math.min(products.length, 1)}-${(pagination.page - 1) * pagination.limit + products.length} of ${pagination.total}`}
                  </Text>
                  <TouchableOpacity style={styles.nextButton} disabled={!pagination.hasNext || loading} onPress={loadMore}>
                    <Text style={[styles.nextText, { opacity: pagination.hasNext ? 1 : 0.5 }]}>Next</Text>
