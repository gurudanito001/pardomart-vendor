...
import { useDocumentPicker, type ImagePickerResult } from '../../../hooks/useImagePicker';
...
async function prepareUploadFile(img: ImagePickerResult, fallbackName: string): Promise<File | any> {
...
export default function UploadDocumentsScreen() {
  ...
  const [certificateImage, setCertificateImage] = useState<ImagePickerResult | null>(null);
  const [idCard, setIdCard] = useState<UploadedFile | null>(null);
  const [idCardImage, setIdCardImage] = useState<ImagePickerResult | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const certFile = await prepareUploadFile(certificateImage, 'store-certificate.jpg');
      ...
      const message = err?.response?.data?.message || err?.message || 'Failed to upload documents.';
      toast.error(message);
    } finally {
  ...
  const UploadZone = ({ onPress, isLoading }: { onPress: () => void; isLoading?: boolean; }) => (
    <TouchableOpacity style={styles.uploadZone} onPress={onPress} disabled={isLoading}>
      <UploadIcon />
      <View style={styles.uploadTextContainer}>
        <Text style={styles.uploadText}>
          Drop Your Images here or <Text style={styles.browseText}>Click to browse</Text>
        </Text>
        <Text style={styles.fileTypesText}>PNG, JPEG and GIF files are allowed</Text>
      </View>
    </TouchableOpacity>
  );

  const UploadedFileCard = ({ file, onRemove }: { file: UploadedFile; onRemove: () => void; }) => (
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.content}>
          <Text style={styles.instructions}>Kindly upload all necessary information and wait for verification</Text>
          ...
          <View style={styles.section}>
            ...
            {certificate ? (
              <UploadedFileCard file={certificate} onRemove={() => handleRemoveFile('certificate')} />
            ) : null}

            {!certificate && (
              <UploadZone onPress={handleStoreCertificateUpload} isLoading={storeCertificatePicker.isLoading} />
            )}
          ...
          <View style={styles.section}>
            ...
            {idCard ? (
              <UploadedFileCard file={idCard} onRemove={() => handleRemoveFile('id')} />
            ) : (
              <UploadZone onPress={handleIdCardUpload} isLoading={idCardPicker.isLoading} />
            )}
